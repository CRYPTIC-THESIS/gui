...
def retrain_data():
    ...
    for i in range(len(crypto)):
        ...
        if(crypto[i]=='BTC'):
            ...
            preds['date'] = pd.Series(eth_date)  
            preds['pred'] = pd.Series(eth_pred)
            ...
            preds['date'] = pd.Series(doge_date)  
            preds['pred'] = pd.Series(doge_pred)
...
try:
    ...
    if (eth_date is not None) and (eth_pred is not None):
        db.add_predictions('ETH_predict',eth_pred,eth_date)
        print('ETH Model Deployed!!!\n\n')
        deployed.append('ETH')

    if (doge_date is not None) and (doge_pred is not None):
